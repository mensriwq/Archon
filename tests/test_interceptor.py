...
try:
    from interceptor_logic import ResponseInterceptor, ENABLE_RESPONSE_INTERCEPT
except ImportError:
    # Fallback if scripts isn't in path
    sys.path.append(os.path.join(os.path.dirname(__file__), ".."))
    from scripts.interceptor_logic import ResponseInterceptor, ENABLE_RESPONSE_INTERCEPT

import interceptor_logic as il_module

CUR_DIR = os.path.dirname(os.path.abspath(__file__))
SAMPLE_JSON = os.path.join(CUR_DIR, "sample_response.json")

def test_interceptor_with_valid_progress():
    il_module.ENABLE_RESPONSE_INTERCEPT = True
    ...
    result = ResponseInterceptor.intercept_response(data)
